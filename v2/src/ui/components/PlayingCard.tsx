...
export default function PlayingCard({
  ...
  if (faceDown) {
    return (
      <div className={`${sz} ${ring} ${opacity} rounded bg-orange-400 border border-orange-300`} />
    );
  }
  if (!card) {
    return (
  ...
  const suit = card[1];
  return (
    <div className={`${sz} ${ring} ${opacity} rounded bg-white border border-neutral-300 flex flex-col items-center justify-center font-bold leading-none ${suitColor(suit)}`}>
      <span>{rank}</span>
